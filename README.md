...
*   **Inicialización de Proyectos**:
    ...
    *   Para Node.js/Express: Crea una estructura de directorios base, incluyendo clases de utilidad (Token, Sanitizer, Validator, HttpException, ErrorHandler), archivos de configuración (`package.json`, `tsconfig.json`, `.env`, `.gitignore`) y archivos de configuración para Express ( `controllers.json` y `services.json`) e instala dependencias básicas.
*   **Generación Rápida de Artefactos**: Permite crear micro-frontends Preact, servicios Express o controladores Express.
*   **Stubs de Backend (para Express)**: Genera automáticamente archivos básicos de Controlador y Servicio Express.
*   **Configuración Automática**:
    ...
    *   Registra el nuevo frontend en un archivo central `.frontforge/frontForgeFronts.json`.
    *   (Opcional y frágil) Intenta registrar assets en `src/apps/environment.ts` del backend.
*   **Compilación Centralizada**: Compila todos los micro-frontends registrados en `.frontforge/frontForgeFronts.json` ejecutando `npm run build` en cada uno de sus directorios (`projectFullPath`).
...
*   `./.frontforge/frontForgeFronts.json`: Archivo JSON que lista los micro-frontends. `frontforge` lo crea/actualiza al usar `create` y lo lee al usar `build`. Formato esperado:
    ```json
    [
      {
    ...
    ```
*   `./.frontforge/express/controllers.json`: Archivo JSON que lista los controladores Express creados con `frontforge create controller`.
    ```json
    [
      {
        "domain": "dominio",
        "feature": "nombreEnCamelCase",
        "path": "src/controllers/dominio/nombreEnCamelCase.controller.ts"
      }
      // ... más entradas
    ]
    ```
*   `./.frontforge/express/services.json`: Archivo JSON que lista los servicios Express creados con `frontforge create service`.
    ```json
    [
      {
        "domain": "dominio",
        "feature": "nombreEnCamelCase",
        "path": "src/services/dominio/nombreEnCamelCase.service.ts"
      }
      // ... más entradas
    ]
    ```
...
*   Crea las carpetas `src/classes`, `src/interfaces`, `src/types`, `public` y el directorio de configuración `.frontforge`. (Nota: `src/routes` se menciona como convención pero no se crea vacío; los stubs de rutas se generan con `create`).
...
*   Crea los archivos de configuración `.frontforge/express/controllers.json` y `.frontforge/express/services.json`.
...
Genera un nuevo micro-frontend Preact, un servicio Express o un controlador Express. Ejecútalo desde la raíz del monorepo.

```bash
npx @aguayodevs-utilities/frontforge create <type> <name> [--port <numero>]
```

*   `<type>`: Tipo de artefacto a crear: `preact`, `service` o `controller`. (Requerido)
*   `<name>`: Nombre/ruta del artefacto. (Requerido)
    *   Para `preact`: Ruta que define la ubicación y nombre del micro-frontend (ej. `admin/user-management`).
        *   Ejemplo: `admin/user-management` creará el frontend en `./fronts/admin/user-management/`.
    *   Para `service` o `controller`: Ruta que define la ubicación y nombre del servicio/controlador (ej. `users/auth`).
        *   Si el proyecto base es Express, creará el stub en `./src/controllers/users/` o `./src/services/users/`.
    *   `feature` se convertirá a `camelCase` para nombres de archivo/clase (ej. `userManagement`).
*   `--port <numero>` (o `-p <numero>`): Puerto para el servidor de desarrollo Vite (solo para `type=preact`). (Opcional, por defecto: `5173`).
...
Compila todos los frontends listados en `.frontforge/frontForgeFronts.json` ejecutando `npm run build` en cada uno de sus directorios (`projectFullPath`). Ejecútalo desde la raíz del monorepo.
...
async function main() {
  ...
  try {
    ...
    await createFrontendPreact('shopping', 'product-detail', { port: 3005 });
