...
export const createService = ({
  ...
  try {
    ...
    // Corregir nombre del servicio para que sea PascalCase
    const serviceName = feature.charAt(0).toUpperCase() + feature.slice(1).replace(/-([a-z])/g, (g) => g[1].toUpperCase()) + 'Service';


    // Corregir los placeholders buscados (sin /* */)
    serviceContent = serviceContent
      .replace(/\${ConstructorCode}/g, codeConstructor) // Corregido
      .replace(/\${MethodCode}/g, codeMethod)         // Corregido
